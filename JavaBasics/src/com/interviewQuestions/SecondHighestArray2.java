...
public class SecondHighestArray2 {

	public static void main(String[] args) {

		int arr[] = {11,11,78,45,23,44,47,4};//length 7//index 0-6
		/*
		...
		for(int number : arr) {
			if (number == first) continue;// is this number equal to the current highest number stored in first
			/*
			 * So it executes continue, which:
			 * Skips all remaining logic inside the loop for this iteration,
			 * Immediately moves on to the next number in the array.
			 * */
			if(number > first) {
				second = first;
				first = number;
